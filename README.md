...
- Tap the right-side **+** beside the terminal tabs to choose an image, screenshot, PDF, or other file. In Direct mode it first opens Compose automatically. The file is E2E-encrypted, saved under `~/Downloads/Termlinks Uploads` on the connected computer, shown as an attachment chip, and its shell-quoted local path is inserted at the cursor so Codex, Claude, a script, or another terminal program can open it. Nothing is sent to the terminal until you press **Enter** or the send arrow.
